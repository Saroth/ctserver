...
#if (CRC32_GENTBL)
unsigned int g_crc32_table[256];   //!< CRC表
/**\brief       初始化CRC表 */
static void crc32_table_init(void)
{
    ...
    for(i = 0; i < 256; i++) {
        c = (unsigned int)i;
        for(j = 0; j < 8; j++) {
            c = ((c & 0x00000001) ? CRC32_POLY : 0) ^ (c >> 1);
        }
        g_crc32_table[i] = c;
    }
}
#else /* Crc32_GenTbl */
unsigned int g_crc32_table[256] = {    //!< crc32表
...
static inline void crc32_table_init(void) {}
#endif /* Crc32_GenTbl */
...
void crc32_update(unsigned int * crc, unsigned char * buffer, unsigned int size)
{
    unsigned int i;
    for(i = 0; i < size; i++) {
        *crc = g_crc32_table[(*crc ^ buffer[i]) & 0xff] ^ (*crc >> 8);
    }
}
/** \brief      结束CRC运算 */
